...
$lessCompilerInstance = false;

function doLessFileCached($fileName)
{
	global $lessCompilerInstance;

	if (!$lessCompilerInstance)
		$lessCompilerInstance = new lessc();

	try {
		// only compile css if new less
		$lessCompilerInstance->autoCompileLess("$fileName.less", "$fileName.css");
	}
	catch (exception $e) {
		echo "PHPLESS Error: " . $e->getMessage();
	}
}
